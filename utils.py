...
def fill_unlabeled_video_segments(start_times:list, end_times:list, labels:list, video_duration:int):
    ...
    if(sum == video_duration):
        return True, corrected_video_action_labels, video_action_labels
    else:
        return False, corrected_video_action_labels, video_action_labels
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(csvs))):
        ...
        for video in videos:
            ...
            _, actions, ba = fill_unlabeled_video_segments(start_times=start_times, end_times=end_times, labels=labels, video_duration=video_duration)
            if(_ == False): 
                print(f"{parsed_df}\n\nBefore:{ba}\n\nCorrected:{actions}\n\n")
